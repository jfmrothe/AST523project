...
plt.hist(logZ)
plt.vlines([cut,np.mean(logZclean),np.mean(logZclean)+np.std(logZclean),np.mean(logZclean)-np.std(logZclean)],0,histheight)
plt.vlines(numres,0,histheight,color='r')
errors = fig.add_subplot(233)
...
nreclhist = fig.add_subplot(232)
plt.xlabel("number of reclusterings / iterations")
plt.hist(Nrecl)
...
niternrecl = fig.add_subplot(236)
plt.ylabel("number of reclusterings")
plt.xlabel("number of iterations")
plt.scatter(Niter,Nrecl)
plt.show()
