...
int main(int argc, char *argv[])
{
    ...
    Samplers sampler(D, N, prior_types, min_vals, max_vals, e); // create sampler object
    ...
    //CH: before that, let's ask dataobj for logL
      ...
      data_obj.logL(Alltheta,nx,ny,logL,nl);

    // discard and resample, get logLmax for convergence check as byproduct
	 sampler.DisgardWorstPoint(logL, nl,nest); 
   sampler.ResetWorstPoint(data_obj); //CH: need to take the data_obj out, and think about how to do that
